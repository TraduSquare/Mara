    ...
    public class Utils
    {
        ...
        public static UWU ReadUWU(string path)
        {
            ...
            for (var i = 0; i < m_file.m_numEntrys; i++)
            {
                ...
                o.m_platform = (MaraConfig.MaraPlatform)_reader.ReadInt32();
        ...
        public static OWO SearchOWO(string path, MaraConfig.MaraPlatform platform)
        {
            var m_file = new UWU();
            var f = File.Open(path, FileMode.Open);
            var _stream = new DataStream(f);
        ...
        public static MaraConfig.MaraPlatform[] GetPlatformsFromUWU(string path)
        {
            var m_file = new UWU();
            var m_platforms = new List<MaraConfig.MaraPlatform>();
            var f = File.Open(path, FileMode.Open);
            var _stream = new DataStream(f);
            var _reader = new DataReader(_stream);
