...
{
    public class MainAot : PatchProcess
    { 
        public MainAot(string oriFolder, string outFolder, string filePath) : base(oriFolder, outFolder, filePath)
        {
        }
        
        public MainAot(string oriFolder, string outFolder, OWO filePath) : base(oriFolder, outFolder, filePath)
        {
        }
        ...
        public override (int, string) ApplyTranslation()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (result.Item1 != 0 && !checkDLC(oriFile))
                    return result;
            }
        ...
        private bool checkDLC(string path)
        {
            ...
            return info.Name.ToUpper().Contains("DLC");
        }
    }
}
